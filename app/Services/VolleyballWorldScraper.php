...
class VolleyballWorldScraper
{
    ...
    public function importarPartidas(): array
    {
        ...
        DB::transaction(function () use ($payload, $times, &$selecoes, $url) {
            foreach ($payload['matches'] as $jogo) {
                $genero = strtolower((string) ($jogo['gender'] ?? '')) === 'women'
                    ? 'feminino'
                    : 'masculino';
                ...
                if (! $casa || ! $fora || ($casa['name'] ?? '') === 'TBD' || ($fora['name'] ?? '') === 'TBD') {
                    continue;
                }
    ...
    private function salvarSelecao(array $time, string $genero, string $sourceUrl): Selecao
    {
        return Selecao::updateOrCreate(
            ...
            [
                'nome' => $time['translatedName'] ?: $time['name'],
                'sigla' => $time['code'] ?: null,
